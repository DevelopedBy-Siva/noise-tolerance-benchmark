...
def save_all_models():
    ...
    splits = load_toxicchat(seed=DEMO_SEED)
    train_texts = splits["train"]["texts"]
    train_labels = splits["train"]["labels"]

    for model_name in ["logreg", "deberta"]:
        ...
        for noise_level in tqdm(NOISE_LEVELS, desc=model_name):
            ...
            if model_name == "logreg":
                save_path = (
                    SAVED_MODELS_DIR / f"logreg_toxicchat_noise_{noise_level}.pkl"
                )
                model.save(str(save_path))
            else:
                save_path = SAVED_MODELS_DIR / f"deberta_toxicchat_noise_{noise_level}"
                model.save(str(save_path))
